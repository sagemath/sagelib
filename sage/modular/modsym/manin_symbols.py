...
class ManinSymbolList(SageObject):
    ...
    def index(self, x):
        """
        Return the index into the list of Manin symbols of x, where x is a
        3-tuple of ints. If x is not in the list, then this function
        returns -1.
        
        INPUT:
        
        
        -  ``x`` - 3-tuple of ints. Something equivalent to an
           element of Manin symbols list, which need not be normalized.
        
        
        OUTPUT:
        
        
        -  ``int`` - the index of the Manin symbol equivalent
           to (i,u,v).
        """
...
class ManinSymbolList_group(ManinSymbolList):
    ...
    def apply(self, j, m):
        ...
        -  ``j`` - integer m = [a, b, c, d] a list of 4
        ...
        -  ``list`` - a list of pairs (j_i, alpha_i), where
           each alpha_i is a nonzero integer, j_i is an integer (the j_i-th
           Manin symbol), and the sum alpha_i\*x_j_i is the image of the
           j-th Manin symbol under the right action of the matrix [a,b;c,d].
           Here the right action of g=[a,b;c,d] on a Manin symbol
           [P(X,Y),(u,v)] is [P(aX+bY,cX+dY),(u,v)\*g].
        """
...
class ManinSymbolList_character(ManinSymbolList):
    ...
    def apply(self, j, m):
        ...
        OUTPUT: a list of pairs (j, alpha_i), where each alpha_i is an
        integer, j is an integer (the j-th Manin symbol), and the sum
        alpha_i\*x_i is the image of self under the right action of the
    ...
    def index(self, x):
        ...
        symbol that is equivalent, modulo a scalar s, to x. Returns the
        index and the scalar.
        ...
        -  ``scalar`` - element of the base field or the int
           0.
        """
        if self._index.has_key(x):
...
class ManinSymbol(SageObject):
    ...
    def apply(self, a,b,c,d):
        ...
        OUTPUT: a list of pairs (alpha_i, x_i), where each alpha_i is an
        integer, x_i is a Manin symbol, and the sum alpha_i\*x_i is the
        image of self under the right action of the matrix [a,b;c,d]. Here
        the right action of g=[a,b;c,d] on a Manin symbol [P(X,Y),(u,v)] is
        [P(aX+bY,cX+dY),(u,v)\*g].
        """
        raise NotImplementedError
    ...
    def lift_to_sl2z(self, N):
        ...
        computes and returns a list [a,b, c',d'] that defines a 2x2 matrix
        with determinant 1 and integer entries, such that c=c'(mod N) and
        d=d'(mod N).
        """
        if N == 1:
            return [1,0,0,1]
    ...
    def modular_symbol_rep(self):
        ...
        symbols. (The result is not cached.)
        """
