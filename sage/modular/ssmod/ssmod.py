...
def Phi2(x,j):
    ...
    157464000000000, 1488*j_pow[2] + 40773375*j_pow[1] + 8748000000, - (j_pow[2]
    - 1488*j_pow[1] + 162000), 1])
...
def Phi3(x,j):
    ...
    8900222976000*j_pow[2] - 1069956*j_pow[3], 452984832000000 +
    8900222976000*j_pow[1] + 2587918086*j_pow[2] + 2232*j_pow[3], 36864000 -
    1069956*j_pow[1] + 2232*j_pow[2] - j_pow[3], 1])
...
def Phi5(x,j):
    ...
    return x.parent()([
        ...
        j_pow[6], 53274330803424425450420160273356509151232000 -
        ...
        + 1665999364600*j_pow[4] + 3720*j_pow[5], 1963211489280 -
        246683410950*j_pow[1] + 2028551200*j_pow[2] - 4550940*j_pow[3]
        + 3720*j_pow[4] - j_pow[5], 1])
...
def Phi7(x,j):
    ...
    The roots of the \emph{modular} polynomial $\Phi_7(x,j)$ are the
    7-isogenous supersingular j-invariants of j.
    ...
    EXAMPLES:
    The following code snippet produces the modular polynomial
    $\Phi_{7}(x,j_{in})$, where $j_{in}$ is a supersingular j-invariant
    defined over the finite field with $7^2$ elements.
        sage: F = GF(7^2, 'a')
        sage: X = PolynomialRing(F, 'x').gen()
        sage: j_in = supersingular_j(F)
        sage: sage.modular.ssmod.ssmod.Phi7(X,j_in)
    ...
    return
    x.parent()([1464765079488386840337633731737402825128271675392000000000000000000*j_pow[2]
    +
    ...
    5208*j_pow[7], 104545516658688000 - 34993297342013192*j_pow[1] +
...
def Phi11(x,j):
    ...
    The roots of the \emph{modular} polynomial $\Phi_{11}(x,j)$ are the
    11-isogenous supersingular j-invariants of j.
    ...
    EXAMPLES:
    The following code snippet produces the modular polynomial
    $\Phi_{11}(x,j_{in})$, where $j_{in}$ is a supersingular j-invariant
    defined over the finite field with $7^2$ elements.
        sage: F = GF(7^2, 'a')
        sage: X = PolynomialRing(F, 'x').gen()
        sage: j_in = supersingular_j(F)
        sage: sage.modular.ssmod.ssmod.Phi11(X,j_in)
    ...
    return
    x.parent()([3924233450945276549086964624087200490995247233706746270899364206426701740619416867392454656000000000000000000000000000000000000
    -
    ...
    + 296470902355240575283200000*j_pow[11] + j_pow[12], -
    ...
    1608331026427734378*j_pow[10] + 8184*j_pow[11],
    296470902355240575283200000 - 374642006356701393515817612*j_pow[1]
    + 27209811658056645815522600*j_pow[2] -
    529134841844639613861795*j_pow[3] +
    4297837238774928467520*j_pow[4] - 17899526272883039048*j_pow[5] +
    42570393135641712*j_pow[6] - 61058988656490*j_pow[7] +
    53686822816*j_pow[8] - 28278756*j_pow[9] + 8184*j_pow[10] -
    j_pow[11], 1])
...
def supersingular_D(prime):
    ...
    EXAMPLES:
    These examples return \emph{supersingular discriminants} for 7,
    15073 and 83401.
...
class SupersingularModule(hecke.HeckeModule_free_module):
    ...
    def hecke_matrix(self,L):
        ...
        NOTES:
            The first list --- list_j --- returned by the
            supersingular_points function are the rows \emph{and}
            column indexes of the above hecke matrices and its ordering should be
            kept in mind when interpreting these matrices.
