...
0. Residue Fields where one has a fast implementation for the arithemtic of the quotient, for example $O_K / p O_K$ for $K$ a number field, $O_K$ it's maximal order and $p$ a prime of $O_K$.  This residue field includes a coercion map from $O_K$.
1. Completions at a places of number fields.  There is a morphism added that maps to the completed ring, which is represented as a finite extension of $\mathbb{Q}_p$
